...
* キー入力や時間経過に反応して呼ばれる関数(コールバック関数)同士の辻褄を合わせること(フラグのようなものを用い、呼ばれる回数や順番を強引に制御した)
* 関数間での情報の遣り取りが難しかった。また、MATLABでは参照型が使えない？のでハンドルで代用し、グローバルな挙動を実現した
* 視覚的な楽しさをある程度確保しつつMATLABが落ちないような処理を書くのが大変だった
